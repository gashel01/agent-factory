...
import { Check, ChevronDown, Eye, ExternalLink, Sparkles, Square, X } from "./icons.js";
...
export function CockpitModal({ onClose }: { onClose: () => void }): JSX.Element {
  ...
  const applyChat = async (): Promise<void> => {
    try { await postJSON("/api/capsule/chat/apply", {}); setDraft(null); toast("Applied."); refreshView(); }
  ...
  const grant = async (c: CapsuleConsent): Promise<void> => {
    try { await postJSON("/api/capsule/consent", { id: c.id }); toast(`${c.title} — provisioned.`); refreshView(); }
    catch (e) { toast(String(e), true); }
  };
    ...
    provPoll((stop) => {
      void fetchJSON<{ state: string; output: string }>("/api/capsule/status?id=__provision__")
        .then((s) => {
          ...
          if (s.state === "ok" || s.state === "error") {
            stop();
            if (s.state === "ok") { toast("Install plan ready — review the facts and approve."); refreshView(); }
    ...
    genPoll((stop) => {
      void fetchJSON<{ state: string; output: string }>("/api/capsule/status?id=__generate__")
        .then((s) => {
          ...
          if (s.state === "ok" || s.state === "error") {
            stop();
            if (s.state === "ok") { toast("Cockpit generated — review it."); refreshView(); }
